...
public interface CommodityEvaluationDao extends BaseMapper<CommodityEvaluation> {
    ...
    Integer evaluationGetByName(String name);

    @Select("select shop_id from commodity  where id=#{id}")
    Integer evaluationGetById(Integer id);
    @Select("select name from sell  where id=#{id}")
    String SellNameGetById(Integer sellerId);
}