...
//            System.out.println("shop:"+shop);
    ...
    public R update(@RequestBody Sell sell) {
        this.sellService.updateById(sell);
        userDao.Update__(sell.getWallet(), sell.getName());
    ...
    public R login(@RequestBody Sell sell) {
        System.out.println("sellservice:" + sellService.login(sell.getName(), sell.getPassword()));
        if ((sell = sellService.login(sell.getName(), sell.getPassword())) != null) {
            if (sell.getStatus() == 1) {
                System.out.println("成功");
                return success(sell);
            } else if (sell.getStatus() == 0) {
                System.out.println("审核中");
                return failed("审核中,请耐心等待！");
            } else {
                System.out.println("失败");
                return failed("商家账号审核失败，请重新提交审核!&nbsp&nbsp&nbsp&nbsp&nbsp <a style='color:red;  margin-top: 5%;' href='#/SellAudit'>申请审核</a>");
            }
            }
